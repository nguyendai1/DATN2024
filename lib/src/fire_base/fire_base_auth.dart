...
class FireAuth {
  ...
  void signUp(String email, String pass, String name, String phone,
    ...
    _firebaseAuth
      ...
      _createUser(userId, email, name, phone, pass, onSuccess, onRegisterError);
      print(userCredential.user);
    }).catchError((err) {
      _onSignUpErr(err.code, onRegisterError);
    });
  ...
  _createUser(String userId,String email, String name, String phone, String pass, Function onSuccess,
      Function(String) onRegisterError) {
    var user = {"email" : email, "name" : name, "phone" : phone, 'pass': pass};
    var ref = FirebaseDatabase.instance.ref().child("users");
    ref.child(userId).set(user).then((user) {
      //success
      onSuccess();
